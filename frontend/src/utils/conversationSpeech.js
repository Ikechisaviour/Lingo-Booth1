...
const HANDS_FREE_COPY = {
  en: {
    unavailable: 'Hands-free mode needs a browser with speech input support.',
    ...
    permission: 'Microphone permission is blocked. Open your browser site settings, allow the microphone, then reload this page.',
    noSpeech: 'No speech was heard. Try again after the listening indicator appears.',
    audioCapture: 'No microphone detected. Plug in or enable a microphone, then try again.',
  ...
  ko: {
    unavailable: '이 브라우저는 음성 입력을 지원하지 않습니다.',
    ...
    permission: '마이크 권한이 차단되어 있어요. 브라우저 설정에서 마이크를 허용한 뒤 다시 시도하세요.',
    noSpeech: '음성이 들리지 않았어요. 듣기 표시가 나온 뒤 말해 주세요.',
    audioCapture: '마이크를 찾을 수 없어요. 마이크를 연결하거나 켠 뒤 다시 시도하세요.',
    network: '음성 인식 중 네트워크 문제가 발생했어요. 연결을 확인하고 다시 시도하세요.',
    aborted: '음성 입력이 취소됐어요.',
    captureFailed: '음성을 인식하지 못했어요. 다시 시도해 주세요.',
  },
  es: {
    unavailable: 'El modo manos libres necesita un navegador con entrada de voz.',
    ...
    permission: 'El permiso del micrófono está bloqueado. Permite el micrófono en la configuración del sitio y recarga la página.',
    noSpeech: 'No se escuchó voz. Habla después de que aparezca el indicador de escucha.',
    audioCapture: 'No se detectó ningún micrófono. Activa o conecta un micrófono e inténtalo de nuevo.',
  ...
  fr: {
    unavailable: 'Le mode mains libres nécessite un navigateur avec saisie vocale.',
    ...
    permission: 'L’autorisation du microphone est bloquée. Autorisez le micro dans les paramètres du site, puis rechargez.',
    noSpeech: 'Aucune voix détectée. Parlez après l’apparition de l’indicateur d’écoute.',
    audioCapture: 'Aucun microphone détecté. Activez ou branchez un microphone, puis réessayez.',
  ...
  de: {
    unavailable: 'Der Freisprechmodus braucht einen Browser mit Spracheingabe.',
    ...
    permission: 'Die Mikrofonberechtigung ist blockiert. Erlaube das Mikrofon in den Website-Einstellungen und lade neu.',
    noSpeech: 'Keine Sprache erkannt. Sprich, nachdem die Höranzeige erscheint.',
    audioCapture: 'Kein Mikrofon erkannt. Mikrofon anschließen oder aktivieren und erneut versuchen.',
...
const FALLBACK_COPY_BY_LANGUAGE = {
  zh: {
...
export function handsFreeCopy(languageCode) {
  ...
  if (FALLBACK_COPY_BY_LANGUAGE[code]) {
    return {
      ...HANDS_FREE_COPY.en,
      ...FALLBACK_COPY_BY_LANGUAGE[code],
      startedStatus: FALLBACK_COPY_BY_LANGUAGE[code].startedSpoken,
      stoppedStatus: FALLBACK_COPY_BY_LANGUAGE[code].stoppedSpoken,
    };
  }
  return HANDS_FREE_COPY.en;
}

export function conversationVoiceForLocale(locale, speaker = '') {
