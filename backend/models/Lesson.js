...
const lessonSchema = new mongoose.Schema({
  ...
  // Learner-facing architecture metadata. These fields align class lessons,
  // quizzes, flashcards, writing, speaking, and listening without forcing a
  // class page to load every linked activity payload.
  learningLevel: {
    type: Number,
    enum: [1, 2, 3, 4],
  },
  ...
  supportLevel: {
    type: String,
    enum: ['native-guided', 'mixed-guided', 'target-first', 'immersion-with-help'],
  },
  quizOptionMode: {
    type: String,
    enum: ['native-assisted', 'target-with-native-hints', 'target-dominant', 'target-first'],
  },
  writingMode: {
    type: String,
    enum: ['handwriting-first', 'type-with-write-option', 'typed-production', 'extended-typed-production'],
  },
  skillStrands: [String],
  lessonRole: {
    type: String,
  ...
  content: [{
    ...
    levelUses: mongoose.Schema.Types.Mixed,
