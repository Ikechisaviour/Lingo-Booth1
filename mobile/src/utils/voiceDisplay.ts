...
type VoiceLike = Record<string, any>;
...
export function voiceId(voice: VoiceLike = {}) {
  return voice.name || voice.ShortName || voice.Name || String(voice || '');
}

function rawVoiceName(voice: VoiceLike = {}) {
...
export function voiceLocale(voice: VoiceLike = {}) {
  return voice.lang || voice.Locale || voice.locale || '';
}

function voiceGenderKey(voice: VoiceLike = {}) {
...
function languageCodeForVoice(voice: VoiceLike = {}, fallbackCode = '') {
...
function localizedRegionName(locale = '', rawName = '', uiLanguage = '') {
  ...
  if (regionCode && typeof Intl !== 'undefined' && (Intl as any).DisplayNames) {
    try {
...
function genderLabelFor(genderKey: string, t: any) {
  if (!genderKey) return '';
  const fallback = genderKey === 'female' ? 'female' : 'male';
  return t(`voicePicker.gender.${genderKey}`, fallback);
}

function localizedLanguageName(code: string, t: any) {
...
export function formatVoiceDisplay(voice: VoiceLike = {}, options: {
  languageCode?: string;
  number?: number;
  t?: any;
  uiLanguage?: string;
...
export function formatVoiceOptions<T extends VoiceLike>(voices: T[] = [], options: {
  languageCode?: string;
  t?: any;
  uiLanguage?: string;
} = {}) {
